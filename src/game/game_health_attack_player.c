...
/*   Updated: 2024/10/30 00:27:36 by mcogne--         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
short	attack_to_player(t_mlx *mlx, t_map *map)
{
	...
	{
		ft_printf("\nPOS PLAYER: %d || POS MONSTER[%d]: %d\n\n", *map->pos_player, i, map->pos_monster[i]);
...
short	draw_health(t_mlx *mlx, t_map *map, t_img *img)
{
	...
	col = ((map->col - 2) / 2) * IMG_WIDTH;
	row = (map->row - 1) * IMG_WIDTH;
	if (map->health_player == 3)
		mlx_put_image_to_window(mlx->mlx_id, mlx->window, img->ui_health[3], col, row);
	else if (map->health_player == 2)
		mlx_put_image_to_window(mlx->mlx_id, mlx->window, img->ui_health[2], col, row);
	else if (map->health_player == 1)
		mlx_put_image_to_window(mlx->mlx_id, mlx->window, img->ui_health[1], col, row);
	else if (map->health_player <= 0)
		mlx_put_image_to_window(mlx->mlx_id, mlx->window, img->ui_health[0], col, row);
	return (1);
}